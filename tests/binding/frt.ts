...
export class frt_proxy__exec_param implements att.ArchetypeType {
    ...
    equals(v: frt_proxy__exec_param): boolean {
        return att.micheline_equals(this.to_mich(), v.to_mich());
    }
    static from_mich(input: att.Micheline): frt_proxy__exec_param {
        return new frt_proxy__exec_param(mich_to_types__e_entrypoints((input as att.Mpair).args[0]), att.Bytes.from_mich((input as att.Mpair).args[1]));
    }
}
...
export const frt_proxy__exec_param_mich_type: att.MichelineType = att.pair_array_to_mich_type([
...
export class Frt {
    ...
    async deploy(owner: att.Address, params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./contracts/frt.arl", {
            owner: owner.to_mich()
        }, params)).address;
        this.address = address;
    }
