...
export class Dgr_proxy {
    ...
    async deploy(owner: att.Address, implementation: att.Address, permits: att.Address, rbac: att.Address, backed_by_metal: string, params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./contracts/dgr_proxy.arl", {
            owner: owner.to_mich(),
            implementation: implementation.to_mich(),
            permits: permits.to_mich(),
            rbac: rbac.to_mich(),
            backed_by_metal: att.string_to_mich(backed_by_metal)
        }, params)).address;
        this.address = address;
    ...
    async get_backed_by_metal(): Promise<string> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
...
export const dgr_proxy = new Dgr_proxy();
