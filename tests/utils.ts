...
import { types__transfer_param, types__transfer_param_mich_type } from './binding/dgr'
import { rec_to_sign_permit_data, rec_to_sign_permit_data_mich_type } from './binding/permits'

export const get_packed_transfer_params = (tps : types__transfer_param[]) : Bytes => {
  const mich = list_to_mich(tps, x => {
    return x.to_mich();
  })
  return (pack(mich, list_to_mich_type(types__transfer_param_mich_type)))
}
