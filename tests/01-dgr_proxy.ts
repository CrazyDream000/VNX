...
import { dgr_proxy, types__operator_param, add_operator, remove_operator, types__transfer_param, types__transfer_destination, types__gasless_param, types__balance_of_request } from './binding/dgr_proxy';
import { dgr } from './binding/dgr';
...
describe('[Proxy] Contracts deployment', async () => {
  it('DGR implementation contract deployment should succeed', async () => {
    await dgr.deploy(alice.get_address(), { as: alice })
  });
  ...
  it('Proxy contract deployment should succeed', async () => {
    await dgr_proxy.deploy(alice.get_address(), dgr.get_address(), permits.get_address(), rbac.get_address(), "gold", {as: alice})
  });
});

describe('[Permits] Contract configuration', async () => {
  it("Add proxy as permit consumer", async () => {
    await permits.manage_consumer(new add(dgr.get_address()), { as: alice })
  })
})

describe('[Proxy] Minting', async () => {
  it('Mint tokens as owner for ourself should succeed', async () => {
    await dgr_proxy.mint(alice.get_address(), new Nat(1000), { as: alice })
    await dgr_proxy.mint(alice.get_address(), new Nat(500), { as: alice })

    const balance_alice_after = await dgr_proxy.get_ledger_value([alice.get_address(), new Nat('0')])
  ...
  it('Mint and burn tokens as owner for ourself should succeed', async () => {
    await dgr_proxy.mint(alice.get_address(), new Nat(1000), { as: alice })
    await dgr_proxy.burn(alice.get_address(), new Nat(500), { as: alice })
  ...
  it('Mint tokens as non owner for ourself should fail', async () => {
    await expect_to_fail(async () => {
      await dgr_proxy.mint(bob.get_address(), new Nat(1000), { as: bob })
    }, dgr.errors.INVALID_CALLER);
  });

  it('Mint tokens as non owner for someone else should fail', async () => {
    await expect_to_fail(async () => {
      await dgr_proxy.mint(carl.get_address(), new Nat(1000), { as: bob })
    }, dgr.errors.INVALID_CALLER);
  });

  it('Mint tokens as owner for someone else should succeed', async () => {
    const balance_carl_before = await dgr_proxy.get_ledger_value([carl.get_address(), new Nat('0')])
    assert(balance_carl_before == undefined, "Invalid amount")

    await dgr_proxy.mint(carl.get_address(), new Nat(1000), { as: alice })

    const balance_carl_after = await dgr_proxy.get_ledger_value([carl.get_address(), new Nat('0')])
    assert(balance_carl_after?.equals(new Nat(1000)), "Invalid amount")
  });

  it('Mint token for user 1', async () => {
    await dgr_proxy.mint(user1.get_address(), new Nat(1), { as: alice })
    const balance_user1_after = await dgr_proxy.get_ledger_value([user1.get_address(), new Nat('0')])
...
describe('[Proxy] Update operators', async () => {
  it('Add an operator for ourself should succeed', async () => {
    const has_operator_before = await dgr_proxy.has_operator_value([dgr_proxy.get_address(), token_id, alice.get_address()])
    assert(has_operator_before == false)
    
    await dgr_proxy.update_operators([
      new add_operator(new types__operator_param(alice.get_address(), dgr_proxy.get_address(), token_id))
    ], { as: alice })
    const has_operator_after = await dgr_proxy.has_operator_value([dgr_proxy.get_address(), token_id, alice.get_address()])
  ...
  it('Remove an existing operator for another user should fail', async () => {
    await expect_to_fail(async () => {
      await dgr_proxy.update_operators([
        new remove_operator(new types__operator_param(alice.get_address(), dgr_proxy.get_address(), token_id))
      ], { as: bob })
    }, dgr.errors.FA2_NOT_OWNER);
  });

  it('Add operator for another user should fail', async () => {
    await expect_to_fail(async () => {
      await dgr_proxy.update_operators([
        new add_operator(new types__operator_param(bob.get_address(), dgr_proxy.get_address(), token_id))
      ], { as: alice });
    }, dgr.errors.FA2_NOT_OWNER);
  });
...
describe('[Proxy] Transfers', async () => {
  it('Transfer simple amount of token', async () => {
    const balance_user1_before = await dgr_proxy.get_ledger_value([user1.get_address(), new Nat('0')])
    const balance_user2_before = await dgr_proxy.get_ledger_value([user2.get_address(), new Nat('0')])
    ...
    await dgr_proxy.transfer([new types__transfer_param(
      user1.get_address(),
    ...
    const balance_user1_after = await dgr_proxy.get_ledger_value([user1.get_address(), new Nat('0')])
    const balance_user2_after = await dgr_proxy.get_ledger_value([user2.get_address(), new Nat('0')])
  ...
  it('Transfer more tokens than owned should fail', async () => {
    ...
    }, dgr.errors.FA2_INSUFFICIENT_BALANCE);
...
describe('[Proxy] Transfers gasless ', async () => {
  it('Transfer gasless simple amount of token', async () => {
    ...
    await dgr_proxy.transfer_gasless([
  ...
  it('Transfer tokens gasless with from different signer should fail', async () => {
    await dgr_proxy.mint(user4.get_address(), new Nat(1), { as: alice })

    const amount = new Nat(1);
    const permit = await permits.get_permits_value(user4.get_address())
    const counter = permit?.counter
    const balance_user4_before = await dgr_proxy.get_ledger_value([user4.get_address(), new Nat('0')])
    const balance_user5_before = await dgr_proxy.get_ledger_value([user5.get_address(), new Nat('0')])
    const chain_id = await get_chain_id();
...
describe('[Proxy] Consume permit', async () => {
  ...
  it('Simple transfer with permit', async () => {
    ...
    await dgr_proxy.transfer(tps, { as: user3 })
  ...
  it('Set expiry with a correct value should succeed', async () => {
    await dgr_proxy.mint(user2.get_address(), new Nat(1), { as: alice })
...
describe('[Proxy] Set metadata', async () => {
  it('Set metadata with empty content should succeed', async () => {
    const metadata_before = await dgr_proxy.get_metadata_value("key")
    assert(metadata_before == undefined);

    await dgr_proxy.set_metadata("key",new Bytes(""), { as: alice })

    const metadata_after = await dgr_proxy.get_metadata_value("key")
    assert(metadata_after?.equals(new Bytes("")));
  });

  it('Set metadata called by not owner should fail', async () => {
    await expect_to_fail(async () => {
      await dgr_proxy.set_metadata("key", new Bytes(""), { as: bob })
    }, dgr.errors.INVALID_CALLER);
  });
  ...
  it('Set metadata with valid content should succeed', async () => {
    ...
    await dgr_proxy.set_metadata("key", data, { as: alice })
...
describe('[Proxy] Burn', async () => {
  it('Burn token should succeed', async () => {
    ...
    await dgr_proxy.burn(user1.get_address(), new Nat(2), { as: alice });
  ...
  it('Burn without tokens should fail', async () => {
    await expect_to_fail(async () => {
      await dgr_proxy.burn(user1.get_address(), new Nat(1), { as: alice });
    }, dgr.errors.FA2_INSUFFICIENT_BALANCE);
  });

  it('Burn tokens with a partial amount of tokens should succeed', async () => {
    const amount = new Nat(500)
    const balance_user1_before = await dgr_proxy.get_ledger_value([carl.get_address(), new Nat('0')])

    await dgr_proxy.burn(carl.get_address(), amount, { as: alice });

    const balance_user1_after = await dgr_proxy.get_ledger_value([carl.get_address(), new Nat('0')])
  ...
  it('Burn tokens with more tokens owned should fail', async () => {
    ...
    await expect_to_fail(async () => {
      await dgr_proxy.burn( carl.get_address(), new Nat(1000), { as: alice });
    }, dgr.errors.FA2_INSUFFICIENT_BALANCE);
...
describe('[Proxy] Freeze', async () => {
  ...
  it('Minting should fail for freezed account', async () => {
    await dgr_proxy.freeze(bob.get_address(), { as: alice });
    await expect_to_fail(async () => {
      await dgr_proxy.mint(bob.get_address(), new Nat(1000), { as: alice })
    }, dgr.errors.FROZEN_ACCOUNT);
  });

  it('Mint for Unfrozen account should success', async () => {
    await dgr_proxy.unfreeze(bob.get_address(), { as: alice });
    await dgr_proxy.mint(bob.get_address(), new Nat(1000), { as: alice });
    const balance_alice_after = await dgr_proxy.get_ledger_value([bob.get_address(), new Nat('0')])
  ...
  it('Burn should fail for freezed account', async () => {
    await dgr_proxy.freeze(bob.get_address(), { as: alice });
    await expect_to_fail(async () => {
      await dgr_proxy.burn(bob.get_address(), new Nat(1000), { as: alice })
    }, dgr.errors.FROZEN_ACCOUNT);
  });

    it('Transfer simple amount of token should fail for a frozen source account ', async () => {
    await dgr_proxy.mint(user1.get_address(), new Nat(1000), { as: alice });    
    await dgr_proxy.freeze(user1.get_address(), { as: alice });
    await expect_to_fail(async () => {
      await dgr_proxy.transfer([new types__transfer_param(
        user1.get_address(),
        [new types__transfer_destination(user2.get_address(), token_id, new Nat(1))])],
  ...
  it('Transfer simple amount of token should fail for a frozen dist account ', async () => {
    await dgr_proxy.unfreeze(user1.get_address(), { as: alice });
    await dgr_proxy.unfreeze(bob.get_address(), { as: alice });
    await dgr_proxy.mint(user1.get_address(), new Nat(1000), { as: alice });    
    await dgr_proxy.freeze(user2.get_address(), { as: alice });
    await expect_to_fail(async () => {
      await dgr_proxy.transfer([new types__transfer_param(
        user1.get_address(),
...
describe('[Proxy] Pause', async () => {
  it('Set DGR on pause should succeed', async () => {
    await dgr_proxy.pause({ as: alice });
    const is_paused = await dgr_proxy.get_paused()
    assert(is_paused);
  });
  ...
  it('Burn is not possible when contract is paused should fail', async () => {
    await expect_to_fail(async () => {
      await dgr_proxy.burn(alice.get_address(), new Nat(1), { as: alice })
    }, dgr.errors.trans_r1);
  });

  it('Unpause by not owner should fail', async () => {
    await expect_to_fail(async () => {
      await dgr_proxy.unpause({ as: bob });
    }, dgr.errors.INVALID_CALLER);
  });

  it('Unpause by owner should succeed', async () => {
    await dgr_proxy.unpause({ as: alice });
...
describe('[Proxy] Transfer ownership', async () => {
  it('Transfer ownership when contract is paused should succeed', async () => {
    const owner = await dgr_proxy.get_owner()
    assert(owner.equals(alice.get_address()));
    await dgr_proxy.pause({ as: alice });
    await dgr_proxy.declare_ownership(alice.get_address(), { as: alice });
    const new_owner = await dgr_proxy.get_owner()
    assert(owner.equals(new_owner));
  });

  it('Transfer ownership as non owner should fail', async () => {
    await expect_to_fail(async () => {
      await dgr_proxy.declare_ownership(bob.get_address(), { as: bob });
    }, dgr.errors.INVALID_CALLER);
  });
  ...
  it('Transfer ownership as owner should succeed', async () => {
    const owner = await dgr_proxy.get_owner()
    assert(owner.equals(alice.get_address()));
    await dgr_proxy.declare_ownership(bob.get_address(), { as: alice })
    await rbac.addBearer(bob.get_address(), new Nat('0'), {as: alice})
    await dgr_proxy.claim_ownership({ as: bob });
    const new_owner = await dgr_proxy.get_owner()
...
describe('[Proxy] Balance of', async () => {
  it('Simple balance of', async () => {
    const balance_alice = await dgr_proxy.get_ledger_value([alice.get_address(), new Nat('0')])
    const balance_of_alice = await dgr_proxy.balance_of(
  ...
  it('Unknown token_id should be zero (based on FA2 standard)', async () => {
    ...
    const balance_of_alice =  await dgr_proxy.balance_of( [new types__balance_of_request(alice.get_address(), fake_token)], { as: alice })
...
describe('[Proxy] Set new Contracts', async () => {
  it('Set new implementation should succeed', async () => {
    await dgr_proxy.unpause({as:bob})
    await dgr_proxy.mint(alice.get_address(), new Nat(1000), { as: bob })
    await dgr.deploy(bob.get_address(), {as: bob})
    await dgr_proxy.set_contract(dgr.get_address(), 'implementation', {as:bob})
    await permits.manage_consumer(new add(dgr.get_address()), {as: alice})
    assert((await dgr_proxy.get_implementation()).toString() === dgr.get_address().toString())
  });

  it('Set new rbac should succeed', async () => {
    await rbac.deploy(alice.get_address(), {as: alice})
    await dgr_proxy.set_contract(rbac.get_address(), 'rbac', {as:bob})
    assert((await dgr_proxy.get_rbac()).toString() === rbac.get_address().toString())
  });
